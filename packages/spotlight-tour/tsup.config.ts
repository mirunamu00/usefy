...
export default defineConfig({
  ...
  async onSuccess() {
    ...
    if (collectedCss.size === 0) {
      // Phase 1 (headless core) ships no SCSS yet — still emit an (empty)
      // dist/styles.css so the "./styles.css" export entry always resolves.
      fs.writeFileSync(
        path.join(distDir, "styles.css"),
        "/* @usefy/spotlight-tour — styles land with the styled component (Phase 2) */\n"
      );
      console.log("No CSS collected from SCSS modules (placeholder styles.css written)");
      return;
    }
...
;(function() {
  ...
  var id = '__usefy_spotlight_tour_styles__';
