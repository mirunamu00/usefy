/**
 * Reproducible bundle-size measurement for @usefy/confetti.
 *
 ...
 *   1. engine-only  — tree-shaken `createConfettiEngine` import
 *   2. headless     — the full `./headless` surface (engine + shapes + presets)
 *   3. react entry  — the full `"."` surface (react/react-dom external)
 *
 * SPEC §4.3 targets: headless < 6 KB gz, React entry < 9 KB gz.
 *
 * Usage: pnpm --filter @usefy/confetti build && pnpm --filter @usefy/confetti size
 */
...
if (!existsSync(resolve(root, "dist/headless.mjs"))) {
  console.error("dist/ not found — run `pnpm --filter @usefy/confetti build` first.");
  process.exit(1);
}
...
console.log("@usefy/confetti bundle sizes (esbuild, minified, ESM):\n");
await measure(
  "engine-only (tree-shaken)",
  'import { createConfettiEngine } from "./dist/headless.mjs"; console.log(createConfettiEngine);',
);
...
console.log("\nSPEC §4.3 targets: headless < 6 KB gz, react entry < 9 KB gz");
const headlessOk = headless < 6 * 1024;
const reactOk = react < 9 * 1024;
