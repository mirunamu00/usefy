---
"@usefy/diff-viewer": minor
---

New package: `@usefy/diff-viewer` — text diff viewer for React with a hand-written, zero-dependency diff engine.
...
- **Two views, one model**: `split` and `unified` render the same `DiffLine` objects; collapsed context travels *inside* the result, so a pre-computed `diff` prop can expand with no source texts
